...
function doInterpolate(float[] cOne, float[] cTwo, float beTwixt) {
...
function doSimpleOutline(float[] myCircle, float[] myAngles) {
  masterX=width/2;
  doDrawCircle(int(masterX), int(masterY), myCircle, myAngles);
}

function coslerp(float val1, float val2, float amt) {
  amt = map (cos(amt*PI), 1.0, -1.0, 0, 1.0);
  return lerp(val1, val2, amt);
}

function doDrawCircle(int xPos, int yPos, float[] ptRad, float[] ptAng) {  
